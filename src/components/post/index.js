...
const Post = (props) => {
  return (
    ...
    >
      <div
        className={props.post.images.length > 0 ? "image" : "no-image"}
        style={{
          backgroundImage:
            props.post.images.length > 0
              ? `url(${getImageUrl(props.post.images[0], imageSize.MEDIUM)})`
              : "",
        }}
      >
      ...
      <div className="content">
        ...
        <div className="extra-info">
          <div className="date">{convertDate(props.post.date)}</div>
          ...
          <div className="city">{props.post.neighborhood.join(" ")}</div>
        </div>
      </div>
      <div className="price">$ {props.post.price}</div>
    </Link>
  );
};
